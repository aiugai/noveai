...
// node_modules 不再自动同步，保留结构供未来扩展缓存目录使用
...
class WorktreeManager {
  constructor() {
    ...
    // 使用当前项目根目录名称作为前缀，格式: <项目名>_issue_
    const projectName = path.basename(this.repoRoot)
    this.prefix = `${projectName}_issue_`
  ...
  async make(issueNumber, options = {}) {
    ...
    logger.info('模式: 不自动同步 node_modules，需在新 worktree 内自行安装依赖')
    ...
    try {
      ...
      logger.info('提示: 进入目录后请按需执行依赖安装命令以初始化 node_modules')
  ...
  async buildWorktree(worktreePath, options = {}) {
    ...
    if (targets.length === 0) {
      logger.info('当前未配置缓存目录，跳过自动同步')
      logger.info('请在新 worktree 内手动执行依赖安装命令后再进行构建')
      logger.info('='.repeat(50))
      return
    }
    if (linkStrategy === 'deps') {
      logger.info('模式: 依赖目录将使用软链接')
    } else if (linkStrategy === 'all') {
      logger.info('模式: 所有可缓存目录将使用软链接')
    }
    logger.info('='.repeat(50))
  ...
  async list() {
    ...
    try {
      ...
      for (const wt of issueWorktrees) {
        // 提取 issue 编号（使用动态前缀匹配）
        const prefixPattern = new RegExp(`${this.prefix.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(\\d+)`)
        const match = wt.path.match(prefixPattern)
  ...
  async getAllIssueWorktrees() {
    try {
      ...
      for (const wt of worktrees) {
        ...
        const pathPattern = new RegExp(`^${escapedPrefix}(\\d+)$`)
        const pathMatch = path.basename(wt.path).match(pathPattern)
