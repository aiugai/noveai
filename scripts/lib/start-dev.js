...
class DevStarter {
  ...
  async showServiceInfo() {
    ...
    const serviceInfo = [
      { service: '后端服务', port: 3005, url: 'http://localhost:3005' },
      { service: '管理后台', port: 3505, url: 'http://localhost:3505' },
      { service: '用户前端', port: 3006, url: 'http://localhost:3006' },
    ]

    logger.ports(serviceInfo)

    logger.info('注意事项:')
    logger.info('- front 固定 3006，admin-front 固定 3505')
